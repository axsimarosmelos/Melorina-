...
for(const name of ['config','content','engine','practice','voice-core','voice-client','voice-ui','app'])html=html.replace('<script src="src/'+name+'.js"></script>',()=>'<script>'+fs.readFileSync(path.join(root,'src/'+name+'.js'),'utf8').replace(/<\/script/gi,'<\\/script')+'</script>');
